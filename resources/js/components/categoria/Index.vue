<template>
     <div class="categoriaPage">
          <banner-interno :category="category" :titleCategory="titleCategory"></banner-interno>
          <div class="container">
...
export default {
     ...
     methods: {
          ...
          async loadApiCategories(){
               try{
                    let sendSolicitud = await axios.get(`/api/v1/list-products/${this.slug}`,)
                    console.log(sendSolicitud)
                    if (sendSolicitud.data.code === 200){
                         this.category = sendSolicitud.data.data_category
                         this.titleCategory = sendSolicitud.data.data_category[0].categoria
