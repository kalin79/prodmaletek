          ...
          <div class="borderFilter">
               ...
               <div class="boxViewOptionFilt">
                    ...
                    <div class="accordion" id="accordionPanelsStayOpenExample">
                         <div class="accordion-item" v-if="colors.length > 0">
                                   ...
                                   <div class="accordion-body">
                                        <a href="javascript:void(0)" class="" v-on:click="setFilterColor(index,element.id)" :id="'checkColor-'+index"  v-for="(element, index) in colors" :key="index">
                                             <div class="d-flex justify-content-start align-items-center">
                                                  ...
                                                  <p>{{ element.name }} ({{ element.count_product }})</p>
                                             </div>
                                        </a>
                                   </div>
                              </div>
                         </div>
                         <div class="accordion-item" v-if="chapas.length > 0">
                         ...
                         <div class="accordion-item" v-if="puertas.length > 0">
                              ...
                              <div id="panelsStayOpen-collapseThree" class="accordion-collapse collapse" aria-labelledby="panelsStayOpen-headingThree">
                                   <div class="accordion-body">
                                        <a href="javascript:void(0)" class="" v-on:click="setFilterPuertas(index,puerta.id)" :id="`checkPuertas-${index}`" v-for="(puerta, index) in puertas" :key="index">
                                             <div class="d-flex justify-content-start align-items-center">
                                                  <div class="boxCheck"></div>
                                                  <p>{{ puerta.name }} Puerta ({{ puerta.count_product }})</p>
                                             </div>
                                        </a>
                                   </div>
                              </div>
                         </div>
                         <div class="accordion-item" v-if="cuerpos.length > 0">
                         ...
                         <div class="accordion-item" v-if="materiales.length > 0">
                              ...
                              <div id="panelsStayOpen-collapseFive" class="accordion-collapse collapse" aria-labelledby="panelsStayOpen-headingFive">
                                   <div class="accordion-body">
                                        <a href="javascript:void(0)" class="" v-on:click="setFilterMateriales(index,element.id)" :id="'checkCuerpos-'+index"  v-for="(element, index) in materiales" :key="index">
                                             <div class="d-flex justify-content-start align-items-center">
                                                  <div class="boxCheck"></div>
                                                  <p>{{ element.name }} ({{ element.count_product }})</p>
                                             </div>
...
export default {
     ...
     computed: {
          ...mapGetters({
               ...
               bandejas: 'category/getFiltroBandejas',
               materiales: 'category/getFiltroMateriales',
               cajones: 'category/getFiltroCajones',
          })
     },
     methods: {
          buscarProductos() {
          ...
          setFilterBandejas(id, value){
               let _element = $(`#checkBandejas-${id}`)
               // console.log(`checkTransmision-${id}`)
               if (_element.hasClass('active')){
                    let i = this.typeBandejas.indexOf(value)
                    if ( i !== -1 ) {
                         this.typeBandejas.splice( i, 1 )
                    }
                    _element.removeClass('active')
               }else{
                    _element.addClass('active')
                    this.typeBandejas.push(value)
               }
               console.log(this.typeBandejas)
               this.filterLoadProdcuts()
          },


          setFilterCajones(id, value){
               let _element = $(`#checkCajones-${id}`)
               // console.log(`checkTransmision-${id}`)
               if (_element.hasClass('active')){
                    let i = this.typeCajones.indexOf(value)
                    if ( i !== -1 ) {
                         this.typeCajones.splice( i, 1 )
                    }
                    _element.removeClass('active')
               }else{
                    _element.addClass('active')
                    this.typeCajones.push(value)
               }
               console.log(this.typeCajones)
               this.filterLoadProdcuts()
          },



          setFilterMateriales(id, value){
               let _element = $(`#checkMateriales-${id}`)
               // console.log(`checkTransmision-${id}`)
               if (_element.hasClass('active')){
                    let i = this.typeMateriales.indexOf(value)
                    if ( i !== -1 ) {
                         this.typeMateriales.splice( i, 1 )
                    }
                    _element.removeClass('active')
               }else{
                    _element.addClass('active')
                    this.typeMateriales.push(value)
               }
               console.log(this.typeMateriales)
               this.filterLoadProdcuts()
          },
          ...
          async filterLoadProdcuts(){
               ...
               data.append('tipoCerradura',this.typeChapas)
               data.append('tipoCantidadPuertas',this.typePuertas)
               data.append('tipoCantidadCuerpos',this.typeCuerpos)
               data.append('tipoMaterial',this.typeMateriales)
               data.append('tipoCantidadCajones',this.typeCajones)
               data.append('tipoCantidadBandejas',this.typeBandejas)


               data.append('slug-categoria', this.doneCategory.slug)

               // this.$store.dispatch('category/consultasAxios',{dataFilters})

               try{
                    // Obtenemos la Prouctos
                    let sendSolicitud = await axios.post(`/api/v1/catalogo`,data)
                    console.log(sendSolicitud)
                    if (sendSolicitud.status === 200){
                         // console.log(sendSolicitud.data.data)
                         this.$store.commit('category/setProductos', sendSolicitud.data)
                    }
               } catch (error) {
                    console.log(error);
               } finally{
